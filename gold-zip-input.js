...
$_documentContainer.innerHTML = `<dom-module id="gold-zip-input">
  ...
  <template id="v0">
    <input is="iron-input" id="input" slot="input" aria-labelledby\$="[[_ariaLabelledBy]]" aria-describedby\$="[[_ariaDescribedBy]]" required\$="[[required]]" validator="zip-validator" type="tel" allowed-pattern="[0-9\\-]" prevent-invalid-input="" maxlength="10" bind-value="{{value}}" autocomplete="postal-code" name\$="[[name]]" disabled\$="[[disabled]]" invalid="{{invalid}}" autofocus\$="[[autofocus]]" inputmode\$="[[inputmode]]" placeholder\$="[[placeholder]]" readonly\$="[[readonly]]" size\$="[[size]]" on-change="_onChange">
  </template>

  <template id="v1">
    <iron-input id="input" slot="input" allowed-pattern="[0-9\\-]" bind-value="{{value}}" invalid="{{invalid}}" validator="zip-validator">
      <input id="nativeInput" aria-labelledby\$="[[_ariaLabelledBy]]" aria-describedby\$="[[_ariaDescribedBy]]" required\$="[[required]]" type="tel" maxlength="10" autocomplete="postal-code" name\$="[[name]]" disabled\$="[[disabled]]" autofocus\$="[[autofocus]]" inputmode\$="[[inputmode]]" placeholder\$="[[placeholder]]" readonly\$="[[readonly]]" size\$="[[size]]" on-change="_onChange">
    </iron-input>
  </template>
...
Polymer({
  ...
  properties: {
     ...
     */
    label: {type: String, value: 'Zip Code'},
  ...
  beforeRegister: function() {
    ...
    var inputTemplate =
        DomModule.import('gold-zip-input', 'template#' + version);
